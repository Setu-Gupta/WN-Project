...
class CSIModelConfig:
    ...
    def build_model(self, n_unit_lstm=200, n_unit_atten=400):
        ...
        x_tensor = tf.keras.layers.Bidirectional(tf.keras.layers.SimpleRNN(activation='sigmoid', units=n_unit_lstm, return_sequences=True))(x_in)
...
if __name__ == "__main__":
    ...
    print("Accuracy:", accuracy_score(np.argmax(y_valid, axis=1), np.argmax(y_pred, axis=1)))
    print("F1 score:", f1_score(np.argmax(y_valid, axis=1), np.argmax(y_pred, axis=1), average='weighted'))
